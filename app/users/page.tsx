...
import { Search, Shield, Loader2, RefreshCw, AlertCircle } from "lucide-react";
...
export default function UsersPage() {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    setError("");
    const { data, error: err } = await users.list(page, LIMIT);
    if (err || !data) {
      setError(err || "Failed to load users. Admin access required.");
    } else {
  ...
  const roleStyle = (role: string) => {
    if (role === "admin") return { bg: "bg-[#7c3aed]", badge: "bg-[#7c3aed]/10 text-[#a78bfa] border-[#7c3aed]/30" };
    return { bg: "bg-[#4a9eff]", badge: "bg-[#4a9eff]/10 text-[#4a9eff] border-[#4a9eff]/30" };
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto relative z-10 px-8 py-6">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 pb-4 border-b border-brand-border">
        ...
        <button onClick={fetchUsers} className="mt-4 sm:mt-0 flex items-center gap-2 px-4 py-3 bg-slate-800 hover:bg-slate-700 border border-brand-border rounded-lg text-white font-semibold text-sm transition-colors">
        ...
        <div className="p-6 border-b border-brand-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-brand-bg/20">
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-[#080b14] border border-brand-border rounded-lg text-sm focus:outline-none focus:border-[#4a9eff] focus:ring-1 focus:ring-[#4a9eff] transition-all w-full sm:w-72"
            />
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-brand-bg/40">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-widest border-b border-brand-border">Email</th>
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-widest border-b border-brand-border text-center">Role</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-brand-border/50 text-sm">
                {filtered.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-brand-bg/60 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-full ${style.bg} flex items-center justify-center font-bold text-sm text-white shadow-sm shrink-0`}>
                            {initials(u.name)}
                          </div>
                      ...
                      <td className="px-6 py-4 font-mono text-slate-500 group-hover:text-[#4a9eff] transition-colors text-xs">{u.id.substring(0, 16)}…</td>
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold tracking-wide border ${style.badge}`}>
